...
struct AppPreferences {
    ...
    DevicePipePreference& pipePreferencesFor(const std::string& serial);
    DevicePipePreference pipePreferencesFor(const std::string& serial) const;
    void save();
    void load();
};
...
struct AppTheme {
    ...
    int mode = 0; // 0=follow Windows, 1=dark, 2=light
    ImVec4 gradientPrimary = ImVec4(0.20f, 0.58f, 1.00f, 1.00f);
    ImVec4 gradientSecondary = ImVec4(0.48f, 0.26f, 0.92f, 1.00f);
    float gradientStrength = 0.72f;
...
private:
    struct UiMessage {
        ...
        bool backupRootProbeComplete = false;
        bool backupRootAvailable = false;
        int backupRootSlot = -1;
        std::string backupRootSerial;
        ...
        float backupProgressAppFraction = -1.0f;
    };
    ...
    bool isOwnMountedDevicePath(const std::string& path, std::string* mountPoint = nullptr) const;
    ...
    void probeBackupManagerRootAccess();
    ...
    int m_backupManagerBackupSelectionAnchor = -1;
    int m_backupManagerJobAppSelectionAnchor = -1;
    ...
    enum class BackupRootAccess { Unknown, Checking, Available, Unavailable };
    ...
    std::vector<float> m_backupAppProgress;
    std::vector<float> m_backupDisplayedAppProgress;
    std::vector<bool> m_backupAppActive;
    int m_backupProgressActiveAppIndex = -1;
    float m_backupDisplayedOverallProgress = 0.0f;
    std::atomic<bool> m_backupPauseRequested{false};
    std::atomic<bool> m_backupCancelRequested{false};
    std::mutex m_backupControlMutex;
    std::condition_variable m_backupControlCV;

    std::mutex m_uiMessageMutex;
    std::deque<UiMessage> m_uiMessages;
    std::unordered_map<ImGuiID, float> m_buttonHoverAnimation;
    float m_shellEntranceAnimation = 0.0f;
    bool m_resolvedLightTheme = false;
    std::chrono::steady_clock::time_point m_lastSystemThemeCheck{};
