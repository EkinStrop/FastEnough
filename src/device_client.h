...
using InstallProgressCallback = std::function<bool(const AppInstallProgress& progress)>;
...
public:
    ...
    bool useRoot() const { return m_useRoot && m_serverIsRoot; }
    ...
    bool installBackupApksBatch(const std::vector<std::string>& backupPaths, const AppBackupOptions& options,
                                std::vector<bool>& installed, std::string& output,
                                ProgressCallback progress = nullptr,
                                InstallProgressCallback installProgress = nullptr);
    ...
    bool backupApp(const std::string& serial, const std::string& packageName, const std::string& appLabel,
    ...
    bool isConnecting() const { return m_startServerActive.load(); }
...
private:
    void stopDeviceServerProcesses(const std::string& serial);
    ...
    bool tryDirectConnect(const std::string& ip, int port = AFM_PORT);
    ...
    std::atomic<bool> m_startServerActive{false};
    ...
    bool m_serverIsRoot = false;
    bool m_ownsServer = true;
